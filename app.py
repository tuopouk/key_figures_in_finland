...
def serve_layout():
    ...
    return dbc.Container([
        ...
        dbc.Row([
            ...
            dbc.Col([
                html.H2('Key figure'),
                dcc.Dropdown(id = 'key_figure_selection',
                             options = key_figures,
                ...
                dcc.Dropdown(id = 'region_selection',
                             options = [{'label':region, 'value':region} for region in data_collection.keys()],
                             value = 'Municipality',
                             multi = False,
                             style = {'font-size':20, 'font-family':'Arial','color': 'black'}
                             ),
                html.H1(id = 'whole-country-header', style = {'textAlign':'center'}, className="mt-5 display-2"),
            ...
            dbc.Col([
                html.H1(id = 'header', style = {'textAlign':'center'}, className="mb-3 mt-3 display-3"),
                dcc.Loading(type = random.choice(['graph', 'cube', 'circle', 'dot' ,'default']), children =dcc.Graph(id = 'region_map', figure = plot_empty_map()))
...
@app.callback(Output('header','children'),Input('key_figure_selection', 'value'),Input('region_selection', 'value') )
def update_header(key_figure, region_level):
    return f"{key_figure} by {region_level}".capitalize()

@app.callback(Output('whole-country-header','children'),Input('key_figure_selection', 'value'))
...
@app.callback(
    ...
    Input('key_figure_selection', 'value'),
